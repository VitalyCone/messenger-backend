...
func NewClient(id string, conn *websocket.Conn, hub *Hub) *Client {
	return &Client{
		Username: id, 
		Conn: conn, 
		send: make(chan model.MessageWS, 256), 
		hub: hub,
	}
}
...
func ServeWS(ctx *gin.Context, username string, hub *Hub) {
	...
	client := NewClient(username, ws, hub)
